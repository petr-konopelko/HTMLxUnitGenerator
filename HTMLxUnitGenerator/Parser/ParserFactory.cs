...
namespace HTMLxUnitGenerator.Parser
{
    internal class ParserFactory
    {
        ...
        public static IParser GetParser(TestRunner testRunner)
        {
            switch(testRunner)
            {
                case TestRunner.Gallio:
                    return new Gallio();
                case TestRunner.JUnit:
                    return new JUnit();
                case TestRunner.MSTest2010:
                    return new MSTest2010();
                case TestRunner.NUnit:
                    return new NUnit();
                default:
                    throw new PlatformNotSupportedException($"Test runner: {testRunner} is unsupported");
            }
        }
        ...
        private bool ValidateJUnitXsd(XmlDocument doc)
        {
            ...
            using (var file = new FileStream(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Schemas\junit.xsd"), FileMode.Open))
            {
